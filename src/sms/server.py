...
def subprocess_server(device, args, log_file, ascii=True):
    """
    Server entry point
    ...
    If ascii is True then it will pass incoming messages as ascii,
    otherwise they are passed as utf-8 encoded text.
    """
    # set up logging
    if log_file:
        logging.basicConfig(level=logging.DEBUG, 
    ...
    while True:
        ...
        for message in m.messages():
            ...
            try:
                text = sms.encoding.decode_unicode(text)
            except ValueError:
                text = sms.encoding.decode_accents(text)
            # encode as ascii or utf-8
            if ascii:
                text = sms.encoding.to_ascii(text)
            else:
